...
def _evaluate(expr: Expr) -> Expr:
    ...
    if evaluated_head.name == "Boole":
        ...
        # Boole is Listable in the kernel; thread over List explicitly.
        if isinstance(argument, Call) and argument.has_head("List"):
            return _evaluated_list_expr(*(
                integer(1 if _truth_value(item) is True else 0)
                if _truth_value(item) is not None
                else call("Boole", item)
                for item in argument.arguments
            ))
