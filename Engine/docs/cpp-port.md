...
- Updated (UTC): 2026-07-28T14:56:32Z
...
Session copy and move operations rebind evaluator callbacks to the destination object. Current-input
`$MessageList` has a separate line-scoped generated-message stream: it retains enabled messages in
order across `$PreRead`, `$Pre`, main evaluation, `$Post`, and `$PrePrint`, even when `Quiet`
suppresses their visible effects, while `Off`-disabled messages enter neither stream. Historical
`MessageList[n]` continues to project only the visible stream. `$PrePrint` runs after that historical
record is finalized, so messages generated by `$PrePrint` remain current-line-only.
