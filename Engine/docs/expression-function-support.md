...
- `ToString`, `ToExpression`, `ToBoxes`, `MakeBoxes`, `MakeExpression`, `StripBoxes`, `SyntaxQ`,
  ...
  `StandardForm` subset. One-argument `ToString[expr]` renders Tungsten's canonical
  `InputForm` for ordinary expressions, but outer display wrappers such as `InputForm[expr]`,
  `FullForm[expr]`, `OutputForm[expr]`, and `StandardForm[expr]` affect one-argument
  `ToString`, `ToBoxes`, REPL output labels, and `Print` text in the same spirit as
  ...
  `MakeExpression[boxes, form]` returns held syntax as `HoldComplete[...]`. FrontEnd-dependent
  formats such as `TraditionalForm`, `TeXForm`, and `MathMLForm` remain out of scope.
...
| `ToString` | `ToString[expr]`, `ToString[expr, InputForm]`, `ToString[expr, StandardForm]` | Converts an evaluated expression to text. `InputForm` and `StandardForm` both use Tungsten's canonical textual renderer rather than byte-for-byte kernel or FrontEnd formatting. The one-argument form renders ordinary expressions as canonical `InputForm`, except outer display wrappers (`InputForm`, `FullForm`, `OutputForm`, `StandardForm`) select payload display text. Explicit form arguments stringify those wrappers literally. | [ToString](https://reference.wolfram.com/language/ref/ToString.html) |
...
| `ToBoxes` | `ToBoxes[expr]`, `ToBoxes[expr, StandardForm]`, `ToBoxes[expr, InputForm]` | Converts the evaluated expression to Tungsten's supported box subset. `StandardForm` returns structural box expressions such as `RowBox`, `FractionBox`, and `SuperscriptBox`; `InputForm` returns a parseable string box. Outermost display wrappers produce Wolfram-style wrapper boxes such as `InterpretationBox`, `TagBox`, `StyleBox`, `FormBox`, and `PaneBox` for the payload display. | [ToBoxes](https://reference.wolfram.com/language/ref/ToBoxes.html) |
