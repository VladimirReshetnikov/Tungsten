...
- `Set` and `SetDelayed` support bare-symbol own values, ordinary compound-LHS down values, and
  curried compound-LHS sub values in Tungsten's process-local symbol registry. `Unset` removes
  bare own values or matching down/sub values. `Attributes[sym] = attrs` is also supported.
  ...
  protect/unprotect mutations. Other assignment-like parser heads such as `TagSet`,
  `TagSetDelayed`, `TagUnset`, `UpSet`, `UpSetDelayed`, `AddTo`, `SubtractFrom`, `TimesBy`, and
  `DivideBy` remain syntax-compatible inert expressions.
