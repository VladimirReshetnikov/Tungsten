...
| `Set` | `Set[sym, rhs]`, parser form `sym = rhs` | For a bare, unprotected symbol `sym`, immediately evaluates `rhs`, stores the resulting own value in the process-local registry (legacy slot and canonical `own_values_definitions` list), and returns that value. Compound left-hand sides such as `f[x_] = ...` route through the LHS classifier and currently raise a clear "compound LHS not yet supported" error; that case is the upcoming compound-LHS pass's entry point. | [Set](https://reference.wolfram.com/language/ref/Set.html) |
| `SetDelayed` | `SetDelayed[sym, rhs]`, parser form `sym := rhs` | For a bare, unprotected symbol `sym`, stores `rhs` *un-evaluated* so each subsequent read of `sym` re-evaluates the body. The canonical own-values entry carries `delayed = True` so future consumers can distinguish the two forms. Compound left-hand sides raise the same not-yet-supported error as `Set`. | [SetDelayed](https://reference.wolfram.com/language/ref/SetDelayed.html) |
| `Unset` | `Unset[sym]`, parser forms `sym =.` and `sym = .` | Removes the bare symbol's own value (and clears the canonical own-values list) and returns `Null`. Protected symbols emit `Unset::wrsym` and return `$Failed`. Other left-hand sides remain unsupported in this subset. | [Unset](https://reference.wolfram.com/language/ref/Unset.html) |
...
| `OwnValues` | `OwnValues[sym]`, `OwnValues["name"]` | Returns own-value rules as `{HoldPattern[sym] :> value}` from the canonical `own_values_definitions` list, falling back to the legacy single-slot field for registry-seeded symbols (e.g., `$MessagePrePrint = Automatic`). Assignment to `OwnValues` is not implemented in this pass; the upcoming compound-LHS pass will lift that restriction. | [OwnValues](https://reference.wolfram.com/language/ref/OwnValues.html) |
