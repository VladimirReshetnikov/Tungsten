...
- `ToString`, `ToExpression`, `ToBoxes`, `MakeBoxes`, `MakeExpression`, `StripBoxes`, `SyntaxQ`,
  ...
  `TeXForm` and `MathMLForm`, and `ToExpression` / `MakeExpression` can read back the TeX and
  MathML strings Tungsten generates for ordinary arithmetic structures. One-argument
  ...
  `StandardForm[expr]`, `TraditionalForm[expr]`, `TeXForm[expr]`, and `MathMLForm[expr]` affect
  one-argument `ToString`, `ToBoxes`, REPL output labels, and `Print` text in the same spirit as
  `wolfram.exe`. Explicit `ToString[InputForm[expr], InputForm]` still stringifies the wrapper
  literally. `ToExpression[boxes]` uses StandardForm box interpretation, while
  `MakeExpression[boxes, form]` returns held syntax as `HoldComplete[...]`. `TeXForm` and
  `MathMLForm` are textual output forms rather than box forms, so `ToBoxes[expr, TeXForm]` and
  `ToBoxes[expr, MathMLForm]` remain unsupported just as in the Wolfram kernel.
...
| `ToString` | `ToString[expr]`, `ToString[expr, InputForm|StandardForm|TraditionalForm|TeXForm|MathMLForm]` | Converts an evaluated expression to text. `InputForm` and `StandardForm` use Tungsten's canonical textual renderer; `TraditionalForm` returns inline-box text; `TeXForm` returns a compact AMS-LaTeX-style subset; `MathMLForm` returns presentation MathML. The one-argument form renders ordinary expressions as canonical `InputForm`, except outer display wrappers select payload display text. | [ToString](https://reference.wolfram.com/language/ref/ToString.html), [TraditionalForm](https://reference.wolfram.com/language/ref/TraditionalForm.html), [TeXForm](https://reference.wolfram.com/language/ref/TeXForm.html), [MathMLForm](https://reference.wolfram.com/language/ref/MathMLForm.html) |
...
| `ToBoxes` | `ToBoxes[expr]`, `ToBoxes[expr, StandardForm|TraditionalForm|InputForm]` plus display wrappers | Converts the evaluated expression to Tungsten's supported box subset. `StandardForm` and `TraditionalForm` return structural box expressions such as `RowBox`, `FractionBox`, `SuperscriptBox`, and `FormBox`; `InputForm` returns a parseable string box. Outermost display wrappers produce Wolfram-style wrapper boxes for `InputForm`, `FullForm`, `OutputForm`, `StandardForm`, `TraditionalForm`, `TeXForm`, and `MathMLForm`. | [ToBoxes](https://reference.wolfram.com/language/ref/ToBoxes.html) |
| `MakeBoxes` | `MakeBoxes[expr]`, `MakeBoxes[expr, StandardForm|TraditionalForm|InputForm]` | Held low-level box conversion. Unlike `ToBoxes`, Tungsten does not evaluate the first argument before boxing it. `TeXForm` and `MathMLForm` are textual output forms, not box forms. | [MakeBoxes](https://reference.wolfram.com/language/ref/MakeBoxes.html) |
