...
| `FactorInteger` | `FactorInteger[n]` for explicit integers and rationals | Returns prime/exponent pairs, including `{-1, 1}` for negative inputs and negative exponents for rational denominators. Partial factorization and Gaussian-integer forms are unsupported. | [FactorInteger](https://reference.wolfram.com/language/ref/FactorInteger) |
...
| `Tr` | `Tr[array]`, `Tr[array, f]` | Sums vector elements or matrix diagonal entries, optionally combining with `f`. Zero-fill sparse matrix traces read only explicit diagonal entries. | [Tr](https://reference.wolfram.com/language/ref/Tr) |
...
| `Ordering` | `Ordering[expr]`, `Ordering[expr, n]`, `Ordering[expr, n, p]` | Returns one-based positions of first-level elements or association values in sorted order, including positive, negative, and `All` counts. | [Ordering](https://reference.wolfram.com/language/ref/Ordering) |
| `Sort` | `Sort[expr]`, `Sort[expr, p]` | Sorts first-level elements or association values while preserving the original head or association keys. | [Sort](https://reference.wolfram.com/language/ref/Sort) |
...
| `ReverseSort` | `ReverseSort[expr]`, `ReverseSort[expr, p]` | Sorts first-level elements into reverse canonical order, or reverses the order induced by a supplied comparator. | [ReverseSort](https://reference.wolfram.com/language/ref/ReverseSort) |
| `SortBy` | `SortBy[expr, f]`, `SortBy[expr, {f1, ...}]`, `SortBy[expr, f, p]`, operator form `SortBy[f]` | Sorts first-level elements by evaluated keys. A scalar key function breaks ties by canonical element order; a list of key functions preserves input order when all keys tie. | [SortBy](https://reference.wolfram.com/language/ref/SortBy) |
| `ReverseSortBy` | `ReverseSortBy[expr, f]`, `ReverseSortBy[expr, {f1, ...}]`, `ReverseSortBy[expr, f, p]`, operator form `ReverseSortBy[f]` | Sorts by evaluated keys in reverse order. Scalar-key ties are reversed by canonical element order, while list-key ties preserve input order. | [ReverseSortBy](https://reference.wolfram.com/language/ref/ReverseSortBy) |
| `OrderingBy` | `OrderingBy[expr, f]`, `OrderingBy[expr, f, n]`, `OrderingBy[expr, f, n, p]`, operator form `OrderingBy[f]` | Returns positions in the order induced by `SortBy`, with positive, negative, and `All` counts. | [OrderingBy](https://reference.wolfram.com/language/ref/OrderingBy) |
