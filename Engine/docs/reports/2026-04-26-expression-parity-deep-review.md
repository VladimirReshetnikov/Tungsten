...
> Post-merge note: this report records the state of the deep-review pass that produced
> commit `61c0498f5a07f64aadee0b53e7d726c576cc2640`. Later attribute work unified several
> items below under registry-backed `Flat`, `Orderless`, `Listable`, and pattern-matching
> semantics. The current support matrix in `docs/expression-function-support.md` is the
> canonical description of present behavior.
...
- **`Boole` Listable threading**: `Boole[{True, False, True}]` now evaluates to
  `{1, 0, 1}`. In the later merged evaluator this is handled by the shared
  registry-backed `Listable` path rather than a `Boole`-specific branch.
