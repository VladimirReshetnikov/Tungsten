...
- Updated (UTC): 2026-07-18T20:57:08Z
- Repository HEAD: f6f7fc62e04f4a39c8182f201f56d370d10ccc90
...
| Sessions | Immutable immediate/delayed symbol own-values, assignment and updates, unset/clear, sequential state, session-aware held control flow, Block-scoped held `Table`/`Do`/`Sum`/`Product` iteration over counts/exact ranges/value lists/nested dependent specifications with nested-list, side-effect-only, or flat arithmetic-fold results as appropriate, and fresh-session stateless command evaluation |
